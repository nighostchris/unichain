...
export async function filterTransaction(params: IFilterEvmTransactionParams): Promise<IFilterTransaction[]> {
  const {
    blockchain, block, trackedAddresses, connection, provider,
  } = params;
  let baseProvider;
  ...
  const commonLog = `filteTransaction (${blockchain}:${block.number}) -`;
  ...
  if (typeof provider !== 'undefined') {
    ...
    baseProvider = new ethers.providers.JsonRpcProvider(connection.endpoint);
  } else {
    baseProvider = ethers.getDefaultProvider();
  }

  for (const transaction of block.transactions) {
    const receipt = await baseProvider.getTransactionReceipt(transaction.hash);
